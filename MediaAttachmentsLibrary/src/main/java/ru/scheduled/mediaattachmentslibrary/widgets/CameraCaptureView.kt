...
import ru.scheduled.mediaattachmentslibrary.databinding.CameraCaptureViewBinding
import ru.scheduled.mediaattachmentslibrary.utils.MEDIA_NOTES_DIRECTORY
import ru.scheduled.mediaattachmentslibrary.utils.isEmulator
            ...
            mockedCl.viewTreeObserver.addOnGlobalLayoutListener(object :
                ...
                override fun onGlobalLayout() {
                    if (mockedCl.height != 0 && mockedCl.width != 0) {
                        ...
                        val mydir = context?.getDir(mockedImagesPath, Context.MODE_PRIVATE)
    ...
    private fun takePhoto() {
        with(binding) {
            ...
            if (saveDestination == SaveLocation.GALLERY) {
                ...
                val dir = context.getDir(MEDIA_NOTES_DIRECTORY, Context.MODE_PRIVATE)
                ...
                GlobalScope.launch(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        launch(Dispatchers.IO) {
                            val fos = FileOutputStream(photoFile)
                            val stream = ByteArrayOutputStream()
                            mbitmap?.compress(Bitmap.CompressFormat.JPEG, imageQuality, stream)
                            val imageBytes = stream.toByteArray()
                            fos.use {
                                it.write(imageBytes)
                            }
        ...
        with(binding) {
            ...
            val mOrientationListener: OrientationEventListener = object : OrientationEventListener(
                ...
                override fun onOrientationChanged(orientation: Int) {
                    ...
                    if (cameraFragmentMainLayout != null && changeCameraIv != null) {
                        when (orientation) {
                            0 -> {
